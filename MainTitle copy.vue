<template>
  <div class="main-title">
    ...
    <div class="scroll">
      <table v-if="titles.data">
        <tr v-for="row in titles.data.values" :key="row.id">
            ...
            <input
              ...
              :class="
                ...
                $store.state.vmixState.activeTitles[
                  titles.data.dataOrder[title.column].titleName
                ] === title.value && 'active'
              "
              type="text"
              autocomplete="off"
              ...
              @blur="blur($event, title)"
              @contextmenu="editElem($event, title)"
              @dblclick="sendTitle($event, title)"
            />
          </td>
...
export default {
  ...
  methods: {
    ...
    //   const newDB = this.titles.values.filter(({ id }) => {
    //     return !this.selectedRows.includes(id);
    //   });
    //   this.titles.values.length = 0;
    //   this.titles.values.push(...newDB);
    //   this.sendToDB();
    // },
    getId() {
      return uuidv4();
    ...
    saveTitle({ target }, { column }) {
      const id = target.id;
      let value = target.value;
      if (this.titles.data.dataOrder[column].upperCase) {
        value = value.toUpperCase();
      }
      const titles = this.titles.data.values.map((row) => {
      ...
      // this.titles.values.length = 0;
      // this.titles.values.push(...newDB);
      this.$store.commit('addTitles', newTitles);
    },
    ...
    sendTitle(event, title) {
      ...
      const overlayInput =
        this.titles.data.dataOrder[title.column].overlayInput; // Находим разрешенный номер overlay для тайтла
      ...
      const currentInput = this.$store.state.vmixState.inputs.filter(
        (input) =>
          input.title === this.titles.data.dataOrder[title.column].titleName
      );
